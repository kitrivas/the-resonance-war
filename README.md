...
**Important:** GitHub Pages must *build* this repo (it's Jekyll source, not finished
HTML). A build workflow is included at `.github/workflows/jekyll.yml`. For it to publish,
set **Settings → Pages → Build and deployment → Source → GitHub Actions** (once). After
that, every push to `main` builds the site and deploys it automatically; watch progress in
the **Actions** tab.

Do **not** use *Deploy from a branch* for this repo — that mode serves the files as-is
without building, which would publish the raw template source instead of finished pages.
